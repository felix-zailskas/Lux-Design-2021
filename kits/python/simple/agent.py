...
from worker import take_worker_action
...
def agent(observation, configuration):
    ...
    for unit in player.units:
        if unit.is_worker() and unit.can_act():
            ...
            action = take_worker_action(player, opponent, unit, game_state, resource_tiles)
            if action:
    ...
    for city in player.cities.items():
        # logging.info(f"Curr City: {city}")
        for city_tile in city[1].citytiles:
            pass
            # logging.info(f"City Tile: {city_tile}")
